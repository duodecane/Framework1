...
void pivot(Vec2 pivot, Vec2& start, float rad) {
    //transform pivot to origin
    start -= pivot;
    ...
    float a = cosf(rad);
    float b = sinf(rad);
    start.x = (start.x * a) - (start.y * b);
    start.y = (start.x * b) + (start.y * a);

    //transform start back
    start += pivot;
}
...
void CApp::RenderScenes() {
    ...
    Vec2 front(0, 0);
    Vec2 back(0, 0);
    float alpha = point2b.x - point1b.x;
    float beta = point1f.y - point1b.y;
    alpha *= 0.5f;
 
    float deltatime = 1.f;// s
    float vnet = 20.f; //v net m/s
    float radiiofturningcurvature = 100.f; //m

    back = Vec2((radiiofturningcurvature-alpha)/ radiiofturningcurvature * vnet, (radiiofturningcurvature + alpha) / radiiofturningcurvature * vnet);
    front = Vec2(sqrtf((radiiofturningcurvature - alpha) * (radiiofturningcurvature + alpha) + beta * beta) / radiiofturningcurvature * vnet,
        sqrtf((radiiofturningcurvature + alpha) * (radiiofturningcurvature + alpha) + beta * beta) / radiiofturningcurvature * vnet);
    bool toright = false;
    for (int i = 0; i < 100; i++) {


        //always turn to the left'
        //pivot is always point 1b
        Vec2 tangent = point2b - point1b;
        float inv = Math::InvSqrt(tangent.x * tangent.x + tangent.y * tangent.y);
        tangent.x *= inv;
        tangent.y *= inv;
        inv = tangent.x;
        tangent.x = -tangent.y;
        tangent.y = inv;
        tangent.x *= back.x * deltatime;
        tangent.y *= back.x * deltatime;
        //tangent is now move in tangent dir
        point1b += tangent;
        point2b += tangent;
        point1f += tangent;
        point2f += tangent;
        ...
        pivotvelo[0] = front;
        pivotvelo[1] = back;
        pivotvelo[0].x -= pivotvelo[1].x;
        pivotvelo[0].y -= pivotvelo[1].x;
        pivotvelo[1].y -= pivotvelo[1].x;

        //first pivot
        pivot(point1b, point2b, (pivotvelo[1].y * deltatime) / radiiofturningcurvature);

        //calculating the angle of front right wheel
        float rad = acosf((alpha*(alpha*alpha*alpha + beta*beta*(alpha+2.f*radiiofturningcurvature) - 6.f*alpha*radiiofturningcurvature*radiiofturningcurvature)) 
            / (2.f * radiiofturningcurvature * radiiofturningcurvature * radiiofturningcurvature * sqrtf(4.f*alpha*alpha+beta*beta))
            )
            - atanf(beta / (2.f * alpha))
            ;

        //pivot(point1b, point1f, (pivotvelo[0].x * deltatime) / radiiofturningcurvature);
        pivot(point1b, point2f, rad); //front right wheel

        Render::FilledCircle(point1b.x, ScreenSize.y - point1b.y, 2.f, Col(255, 0, 0), 10);
        Render::FilledCircle(point2b.x, ScreenSize.y - point2b.y, 2.f, Col(0, 0, 255), 10);
      //  Render::FilledCircle(point1f.x, ScreenSize.y - point1f.y, 2.f, Col(0, 255, 0), 10);
        Render::FilledCircle(point2f.x, ScreenSize.y - point2f.y, 2.f, Col(255, 255, 255), 10);
